...
export function useDocumentThumbnail(id: string | number, options = {}): UseQueryResult<string | undefined, HttpError> {
  ...
  const objectUrl = useMemo(() => {
    ...
    const cached = blobUrlCache.get(query.data);
    if (cached) return cached;
    const created = URL.createObjectURL(query.data);
    blobUrlCache.set(query.data, created);
    blobUrlReverseCache.set(created, query.data);
    return created;
  }, [query.data]);
    ...
    return () => {
      ...
      if (next <= 0) {
        ...
        const timer = globalThis.setTimeout(() => {
          blobUrlRevokeTimers.delete(objectUrl);
          const blob = blobUrlReverseCache.get(objectUrl);
          if (blob) {
