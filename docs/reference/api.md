...
!!! note "Generated OpenAPI specification"
    The running API serves an autogenerated OpenAPI specification at `GET /api/v1/openapi.json` with interactive Swagger UI at `/api/docs`. The specification describes every endpoint below and is the authoritative contract; the tables that follow document user-visible behavior in more depth.
