...
When an upload requests virus scanning, the API stores the file and enqueues a virus scan job instead of the normal post-upload processing jobs. The worker marks the file `scanning`, streams the stored object to the configured `clamd` endpoint, and records the verdict:
...
If post-upload enqueueing fails after a clean scan, the file is reset to `scan_error` so the next retry can recover instead of leaving it stuck.  ClamAV signature databases are large and take time to initialize, so avoid sending scan jobs to ClamAV before it is ready.
